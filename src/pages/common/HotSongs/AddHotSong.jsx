    ...
    useEffect(() => {
        if (searchQuery.length > 0) {
            const fetchSearchResults = async () => {
                try {
                    setLoading(true);
                    const response = await axios.post('http://localhost:3007/api/v1/user/search-lyricfind', {
                        type: 'track',
                        query: searchQuery,
                        page: currentPage,
                        pageSize: 10
                    ...
                    const { tracks } = response.data.data.lyricfind;
                    if (tracks && tracks.track) {
                        setSearchResults(tracks.track);
                        setTotalPages(tracks.$.totalpages);
                    } else {
                        setSearchResults([]);
                        setTotalPages(1);
                    }
